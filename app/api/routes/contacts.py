...
async def get_contacts(request: Request, db=Depends(get_mongo_db)):
    ...
    contacts = [{**convo, "_id": str(convo["_id"])} async for convo in contacts_cursor]

    return contacts

@contacts_router.get("/add_contact/{contact_email}")
async def add_contacts(request: Request, contact_email:str, db=Depends(get_mongo_db), auth_db=Depends(get_db)):
    user_data = request.state.user
    user_id = user_data["sub"]
    ...
    new_contact = await db.contacts.insert_one({"user_id": user_id,
